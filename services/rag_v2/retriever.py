...
# Module-level cache cho ChromaDB collection và index payload
_index_cache: dict[str, dict] = {}


def _get_cached_index(index_path: Path) -> dict:
    """Load index một lần, cache lại cho các request tiếp theo."""
    cache_key = str(index_path)
    if cache_key in _index_cache:
        return _index_cache[cache_key]
    payload = _load_index_fresh(index_path)
    _index_cache[cache_key] = payload
    return payload


def invalidate_index_cache() -> None:
    """Gọi sau khi rebuild index."""
    _index_cache.clear()


def _load_index_fresh(index_path: Path) -> dict:
    from .indexer import get_chroma_client
    if not index_path.exists():
        raise RuntimeError(f"rag_v2 index not found: {index_path}")
    payload = json.loads(index_path.read_text(encoding="utf-8"))
    client = get_chroma_client()
    try:
        collection = client.get_collection(name=RAG_V2_INDEX_NAME)
        payload["chroma_collection"] = collection
...
def retrieve(question: str, *, top_k: int = 5, index_path: Path | None = None, filters_override: dict[str, set[str]] | None = None) -> dict:
    ...
    collection = index_payload.get("chroma_collection")
    if not collection:
    ...
    # Detect model and embed question
    ...
    query_vec = _normalize_vector(query_embedding).tolist()
    
    # Build Chroma Where clauses based on inferred filters (if any)
    chroma_where = {}
    where_conditions = []
    
    if filters.get("doc_types"):
        where_conditions.append({"doc_type": {"$in": list(filters["doc_types"])}})
    if filters.get("products"):
        where_conditions.append({"product": {"$in": list(filters["products"])}})
    if filters.get("source_groups"):
        where_conditions.append({"source_group": {"$in": list(filters["source_groups"])}})
    ...
    # Query Chroma (Dense Search)
    # We retrieve more chunks (e.g. 30) for re-ranking
    query_args = {
    ...
    if chroma_where:
        query_args["where"] = chroma_where
        
    try:
        results = collection.query(**query_args)
    except Exception:
        # Fallback without where if filtering fails
    ...
    if not results or not results["documents"] or not results["documents"][0]:
        return {"items": [], "filters": filters, "index_name": index_payload.get("index_name") or ""}
        
    # Reconstruct item list from Chroma results
    chroma_docs = results["documents"][0]
    chroma_metas = results["metadatas"][0]
    chroma_distances = results["distances"][0]
    
    working_set = []
    for doc, meta, distance in zip(chroma_docs, chroma_metas, chroma_distances):
        item = dict(meta)
        item["text"] = doc
        # ChromaDB cosine distance ∈ [0, 2]: 0 = identical, 2 = opposite
        # Cosine similarity = 1 - distance (khi collection dùng hnsw:space=cosine)
        item["dense_score"] = max(0.0, min(1.0, 1.0 - float(distance)))
        working_set.append(item)

    # Enforce inferred/intent filters after Chroma query as a second guard. If
    # Chroma where-filter fails and we fall back to an unfiltered vector query,
    # product chunks must not leak into small-talk or other scoped intents.
    working_set = _apply_filters(working_set, filters)

    if not working_set:
         return {"items": [], "filters": filters, "index_name": index_payload.get("index_name") or ""}
    
    # BM25 Reranking over the retrieved subset
    tokenized_corpus = [tokenize(normalize_text(item.get("text", ""))) for item in working_set]
    ...
    for idx, item in enumerate(working_set):
        ...
        # Normalize BM25 locally
        max_bm25 = max(bm25_scores) if len(bm25_scores) > 0 else 1.0
        if max_bm25 <= 0:
            max_bm25 = 1.0
        lexical_score = float(bm25_scores[idx]) / max_bm25
        ...
        # New Hybrid Score formulation: combining dense, bm25, keywords and intent bonuses
        final_score = (dense_score * 0.5) + (lexical_score * 0.3) + (keyword_score * 0.2) + doc_type_bonus + faq_match_bonus
