...
def estimate_is_upside_down_debug(rows, img_h):
    ...
    if bot_contact > top_contact and top_name > bot_name: decisions.append("UPRIGHT_CON_BOT_NAM_TOP")
    if top_contact > bot_contact and bot_name > top_name: decisions.append("UPSIDE_DOWN_CON_TOP_NAM_BOT")
    if top_contact > bot_contact and bot_name >= top_name: decisions.append("UPSIDE_DOWN_FALLBACK")
    
    print(f"Decisions triggered: {decisions}")
    return len(decisions) > 0

def main():
    image_names = ["REG_12-03-2026_21-33-15_bcard.jpeg"]
    reader = get_reader()
